...
long m2LoggerTask (memMap *ptr)
{
   ...
   if (timeNow (&timeStamp) != OK)
   {
      errlogSevPrintf(errlogMajor, "m2loggerTask - error reading timeStamp\n");
   }
...
epicsExportRegistrar(CADlogTogRegisterCommands);
epicsExportAddress(int, CADlogging);
epicsRegisterFunction(cadDirLog);
